...
def train(model_filepath, feature_data_filepath='data/features'):
    ...
    onnx_model = keras2onnx.convert_keras(model, f'{model.name}')
    onnx.save_model(onnx_model, f'{model.name}.onnx', location=model_filepath)
